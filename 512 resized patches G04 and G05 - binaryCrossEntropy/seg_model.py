...
# define model
model1 = sm.Unet(BACKBONE1, encoder_weights='imagenet', classes=n_classes, activation=activation, input_shape=(512, 512, 3))
...
import visualkeras.layer_converters

class CustomLayerConverter(visualkeras.layer_converters.LayerConverter):
    def get_color(self, layer):
        custom_colors = {
            'Conv2D': (0, 255, 0),
            'MaxPooling2D': (0, 255, 255),
            'Dense': (255, 0, 255),
            'Flatten': (255, 165, 0),
            'Dropout': (0, 0, 255),
            'BatchNormalization': (255, 0, 0),
            'InputLayer': (255, 255, 0)
        }
        layer_name = layer.__class__.__name__
        return custom_colors.get(layer_name, super().get_color(layer))
...
custom_colors = {
    ...
    'Flatten': 'orange',
    'Concatenate': 'blue',
    'BatchNormalization': 'red',
    'InputLayer': 'black'
}


# Customize the font if you'd like
font = ImageFont.truetype("arial.ttf", 200)  # using arial with size 20

# Visualize the model
visualkeras.layered_view(model1, legend=True, to_file='model_layers.png', font=font, draw_volume=True)


visualkeras.layered_view(
    model1,
    legend=True,
    to_file='model_layers.png',
    font=font,
    draw_volume=True,
